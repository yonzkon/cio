...
static void *client_thread(void *args)
{
    ...
    *colon = 0;
    ...
    cio_register(ctx, fd, CIOF_T_CONNECT, CIOF_READABLE | CIOF_WRITABLE);
    for (;;) {
        if (client_finished)
            break;
            ...
            switch (cioe_get_fd_type(ev)) {
                case CIOF_T_CONNECT: {
                    ...
                    if (code == CIOE_WRITABLE) {
                        ...
                        cio_register(ctx, fd, CIOF_T_CONNECT, CIOF_READABLE);
...
static void *server_thread(void *args)
{
    ...
    *colon = 0;
    ...
    struct cio *ctx = cio_new();
    cio_register(ctx, fd, CIOF_T_LISTEN, CIOF_READABLE);
    for (;;) {
        if (server_finished)
            break;
        assert_true(cio_poll(ctx, 100 * 1000) == 0);
            ...
            switch (cioe_get_fd_type(ev)) {
                case CIOF_T_LISTEN: {
                    ...
                    if (code == CIOE_READABLE) {
                        ...
                        cio_register(ctx, new_fd, CIOF_T_ACCEPT, CIOF_READABLE);
                    }
                    break;
                }
                case CIOF_T_ACCEPT: {
