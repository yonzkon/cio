...
static void *client_thread(void *args)
{
    ...
    struct unix_stream *stream = unix_stream_connect(UNIX_ADDR);
    assert_true(stream);

    struct cio *ctx = cio_new();
    cio_register(ctx, unix_stream_get_raw(stream),
        ...
        for (;;) {
            ...
            switch (cioe_get_fd_type(ev)) {
                case CIOF_T_CONNECT: {
                    ...
                    struct unix_stream *stream = (struct unix_stream *)cioe_get_wrapper(ev);
                    if (code == CIOE_WRITABLE) {
                        char *payload = "from client";
                        int nr = unix_stream_send(stream, payload, strlen(payload));
...
static void *server_thread(void *args)
{
    ...
    struct unix_listener *listener = unix_listener_bind(UNIX_ADDR);
    assert_true(listener);

    struct cio *ctx = cio_new();
    cio_register(ctx, unix_listener_get_raw(listener),
            ...
            switch (cioe_get_fd_type(ev)) {
                case CIOF_T_LISTEN: {
                    ...
                    struct unix_listener *listener =
                        (struct unix_listener *)cioe_get_wrapper(ev);
                    if (code == CIOE_READABLE) {
                        struct unix_stream *new_stream =
                            unix_listener_accept(listener);
                        cio_register(ctx, unix_stream_get_raw(new_stream),
                ...
                case CIOF_T_ACCEPT: {
                    int code = cioe_get_code(ev);
                    struct unix_stream *stream =
                        (struct unix_stream *)cioe_get_wrapper(ev);
                    if (code == CIOE_READABLE) {
                        char buf[256] = {0};
                        int nr = unix_stream_recv(stream, buf, sizeof(buf));
                        if (nr == 0 || nr == -1) {
                            ...
                            unix_stream_send(stream, payload, strlen(payload));
